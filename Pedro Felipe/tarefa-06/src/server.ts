...
// 1. LISTAR TODOS (GET)
fastify.get('/tarefas', async () => tarefas);

// 2. BUSCAR POR ID (GET)
fastify.get<{ Params: { id: string } }>('/tarefas/:id', async (request, reply) => {
...
// 3. CRIAR (POST)
fastify.post<{ Body: { titulo: string } }>('/tarefas', async (request, reply) => {
...
fastify.put<{
...
}>('/tarefas/:id', async (request, reply) => {
  const { id } = request.params;
  ...
  tarefas[index] = { ...tarefas[index], titulo: titulo ?? tarefas[index].titulo, concluida: concluida ?? tarefas[index].concluida };
  return tarefas[index];
});

// 5. REMOVER (DELETE)
fastify.delete<{ Params: { id: string } }>('/tarefas/:id', async (request, reply) => {
  const { id } = request.params;
